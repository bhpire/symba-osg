seednoise         = 1e8 #seed for all random realizations of the first synthetic dataset, for each run on an OSG node, it is incremented by 1
realizations      = 5 #number of different realizations for noise and corruptions for each model
...
mod_rotation      = [0,30,60,90] #rotate models by this amount [deg], can be a list
...
smodel_kywrds     = ['Sa+0.5', 'i_10_PA_0', 'Rhigh_160'] #if input_models=[], all filenames with these keywords in storage_filepath0/+mdir are used as input models
