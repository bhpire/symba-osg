seednoise         = 2000000 #seed for all random realizations of the first synthetic dataset, for each run on an OSG node, it is incremented by 1
...
frameduration     = '600' #duration of a single model frame in s; TODO: read this from the library itself
mod_rotation      = 0 #rotate models by this amount [deg]
...
mod_num_select    = [] #set to [] to load all models (time-dependent src) or [a,b,c,...] to load models#a,#b#,#c,... (start counting them at 0) for separate synthetic datasets
