seednoise         = 100 #seed for all random realizations of the first synthetic dataset, for each run on an OSG node, it is incremented by 1
...
smodel_kywrds     = ['Sa+0.5', 'Rhigh_1/'] #if input_models=[], all filenames with these keywords in storage_filepath0/+mdir are used as input models
...
rand_mod_num_sel  = 5 #if >0, overwrite mod_num_select to draw this many random model files for each time-dependent model
input_models      = []
