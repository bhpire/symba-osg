seednoise         = 1000 #seed for all random realizations of the first synthetic dataset, for each run on an OSG node, it is incremented by 1
...
rand_mod_num_sel  = 30 #if >0, overwrite mod_num_select to draw this many random model files for each time-dependent model
input_models      = []
