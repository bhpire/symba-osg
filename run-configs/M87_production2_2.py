seednoise         = 185000 #seed for all random realizations of the first synthetic dataset, for each run on an OSG node, it is incremented by 1
realizations      = 1 #number of different realizations for noise and corruptions for each model
...
tracks            = ['e17e11'] #Can obs M87 on a10,b06,d05,e11 and SGRA also on c07
...
rand_mod_num_sel  = 500 #if >0, overwrite mod_num_select to draw this many random model files for each time-dependent model
input_models      = []
